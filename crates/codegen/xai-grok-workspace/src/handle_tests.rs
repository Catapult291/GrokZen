...
async fn update_tool_config_rejects_turn_started_during_resolve() {
    ...
    local
        .run_until(async {
            let late_rejected_before =
                swap_rejected_count("turn_active_late", "update_tool_config");
            let handle = make_handle();
            let session = handle.session("main").expect("main session exists");
            let toolset_before = session.toolset();
    ...
    local
        .run_until(async {
            ...
            let parent_ids: Vec<String> =
                parent_baseline.tools.iter().map(|t| t.id.clone()).collect();
            let child = handle
                .fork_session(fork_cfg_with(
                    "child",
                    CapabilityMode::ReadWrite,
            ...
            let child_ids: Vec<String> =
                child_baseline.tools.iter().map(|t| t.id.clone()).collect();
...
fn fork_session_concurrent_same_id_only_one_winner() {
    // The workspace actor is !Send, so the 16 racers have to run as tasks on one
    // LocalSet rather than as OS threads each blocking on its own runtime — a thread
    // entering the multi_thread runtime still cannot `spawn_local`. Concurrency is
    // preserved: all 16 forks are in flight at the same time on the same handle.
    ...
    local.block_on(&rt, async {
        ...
        for _ in 0..16 {
            ...
            // Borrow through the `Arc` that outlives `racers`; cloning per iteration would
            // drop the temporary at the end of the statement while the future still borrows it.
            racers.push(handle.fork_session(c));
        }
        let mut wins = 0;
        let mut losses = 0;
        for res in futures::future::join_all(racers).await {
            match res {
                Ok(_) => wins += 1,
...
async fn a_failed_server_is_retried_by_the_next_convergence() {
    ...
    local
        .run_until(async {
            ...
            .unwrap();
            let delta =
                converge_with(&handle, "failed-mcp", &hub, crate::mcp::McpReclaim::Always).await;
    ...
    local
        .run_until(async {
            ...
            let (second_url, second_task) =
                spawn_bind_mcp_server(BindMcpTestState::default()).await;
            let factory = Arc::new(TestSessionContextFactory::new());
            let mut config =
                WorkspaceHandle::test_config(factory.temp.path().to_path_buf(), factory.clone());
            config.bind_mcp = Some(BindMcpConfig::new([
...
async fn a_soft_rebind_does_not_queue_behind_an_in_flight_convergence() {
    ...
    local
        .run_until(async {
            ...
            let rebound =
                tokio::time::timeout(std::time::Duration::from_secs(2), resolver(sid, None))
                    .await
                        ...
                        if !fired_hook.swap(true, std::sync::atomic::Ordering::SeqCst) {
                            ...
                            tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current()
                                    .block_on(handle.teardown_session_mcp("stale"))
                            });
                        }
                        Ok(())
                    },
...
async fn a_stale_drive_commit_cannot_enter_a_revived_life() {
    ...
    local
        .run_until(async {
            ...
            let (tx, mut rx) =
                tokio::sync::mpsc::channel(crate::config::BindMcpConfig::MAX_SERVERS);
            let drive = {
                let session = Arc::clone(&session);
                let server = configured_test_mcp("gated", url);
                tokio::spawn(async move {
                            ...
                            tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current().block_on(async move {
                                    ...
                                    *session.mcp_native_tool_ids.lock() =
                                        std::collections::HashSet::from([ToolId::new(
                                            "winner_marker",
                                        )
                                        .unwrap()]);
                                })
                ...
                crate::path_virtualization::BindMountHook::probe_then_mount(
                    ...
                    move |_root| {
                        if !fired_hook.swap(true, std::sync::atomic::Ordering::SeqCst) {
                            ...
                            tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current().block_on(async move {
                                    handle.teardown_session_mcp("no-mcp").await
                                })
                            });
                        }
                        Ok(())
...
async fn an_empty_configured_set_never_fails_a_bind() {
    ...
    local
        .run_until(async {
                ...
                crate::path_virtualization::BindMountHook::probe_then_mount(
                    ...
                    move |_root| {
                        if !fired_hook.swap(true, std::sync::atomic::Ordering::SeqCst) {
                            ...
                            tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current().block_on(async move {
                                    handle.teardown_session_mcp("empty-cfg").await
                                })
                            });
                        }
                        Ok(())
            ...
            let metadata =
                serde_json::json!({"metadata": {"session_root": "/workspace/empty-cfg"}});
            resolver(sid.clone(), Some(metadata))
                .await
                ...
                crate::path_virtualization::BindMountHook::probe_then_mount(
                    ...
                    move |_root| {
                        if !fired_hook.swap(true, std::sync::atomic::Ordering::SeqCst) {
                            ...
                            tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current().block_on(async move {
                                    handle.teardown_session_mcp("racy").await
                                })
                            });
                        }
                        Ok(())
...
async fn bind_mcp_discovery_is_concurrent_and_bounded() {
    ...
    local
        .run_until(async {
            ...
            let delta =
                converge_with(&handle, "bounded-mcp", &hub, crate::mcp::McpReclaim::Always).await;
            assert!(
