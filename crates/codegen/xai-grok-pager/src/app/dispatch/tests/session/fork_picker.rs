...
#[test]
fn at_flag_skips_the_picker_and_cuts_at_the_position() {
    let mut app = fork_test_app_with_prompts(&["first", "second", "third"]);
    ...
    assert!(
        ...
        "`--at 2` forks before prompt 2, keeping prompt 0 and 1: {effects:?}"
    );
    assert_eq!(app.agents[&AgentId(1)].prompt.text(), "third");
}
