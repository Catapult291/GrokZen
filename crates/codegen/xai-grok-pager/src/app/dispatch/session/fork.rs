...
pub(in crate::app::dispatch) fn dispatch_fork_resolved(
    ...
    let parent_marker = match (cut.target_prompt_index, directive.as_deref()) {
        // The cut keeps prompts `0..=target`, so the branch point is the prompt after it.
        (Some(target), _) => localized_template(
            locale.as_ref(),
            "session.fork.parent_marker.at",
            "Forked before prompt #{index}",
            &[("{index}", &(target + 1).to_string())],
        ),
    ...
    app.agents.insert(new_id, new_agent);
    {
        ...
        // Forking before a prompt returns that prompt to the composer so the user can rewrite it,
        // which is the point of picking an earlier fork point at all.
