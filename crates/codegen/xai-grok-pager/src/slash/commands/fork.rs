...
pub struct ForkArgs {
    ...
    /// `Some(k)` -> fork before one-based user prompt position `k`; position 1 is intentionally rejected
    /// because the wire protocol cannot represent an empty cut. `None` -> keep the whole conversation
    /// (the default and the picker's "current state" row).
    pub at_prompt: Option<usize>,
}

/// Where the fork's copy of the parent conversation ends.
///
/// The shell's `target_prompt_index` keeps every prompt up to and including the index. Because
/// `--at k` is one-based and the first prompt has index 0, "fork before prompt `k`" uses wire value
/// `k - 2`; position 1 is rejected rather than silently copying one prompt.
