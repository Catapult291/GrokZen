...
fn rows_contain_categories_and_settings_through_pr_14() {
    ...
    #[cfg(windows)]
    expected.push("default_shell".into());
    // SHELL-owned hunk_tracker_mode (Advanced; `off` disables it).
    expected.push("hunk_tracker_mode".into());
    assert_eq!(settings, expected);
    #[cfg(windows)]
    assert_eq!(
        settings.get(settings.len() - 2),
        Some(&"default_shell"),
        "Windows must expose default_shell immediately before hunk_tracker_mode"
    );
