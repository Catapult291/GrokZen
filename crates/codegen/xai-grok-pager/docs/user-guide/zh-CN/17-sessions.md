...
接受参数。选择器最多请求 100 个会话；默认列表会隐藏空壳会话，但显式搜索仍保留
匹配项，隐藏列表项不会删除本地磁盘历史。
...
```
/fork [--worktree|--no-worktree] [--at <prompt>] [directive]
```

可选的 `directive` 会设置新会话的第一条提示。使用 `--worktree` 或 `--no-worktree`
选择分叉是否在新的 git worktree 中运行；省略两者则每次询问。使用 `--at <prompt>` 可从指定的第一条用户提示之前分叉，该提示会预填到新会话中。位置 `1` 不提供，因为它只会得到空会话；请改用 `/new`。省略 `--at` 时会打开分叉点选择器。
...
`/rewind` 把会话回退到较早的轮次。具体回退什么，取决于你在确认框里的选择：可以只
回退对话、只回退文件，或两者一起回退。`/undo` 是只回退对话的形式：同样列回退点，
但不弹确认，也不动文件。
...
运行 `/rewind` 时（或者在提示为空、有对话消息且空闲时，于 800 毫秒内按 **Esc
Esc**），Grok 会：

1. 显示回退点列表（每条用户提示一个，从旧到新），光标默认停在最新的一条
2. 让你选择要回退到的点——光标所在的那一轮会滚到终端顶部，并在时间线侧栏上高亮；**Esc** 关闭列表，终端回到打开前的位置
3. 开启**回退前确认**（`/settings` 中的默认设置）时，先问你要回退什么，再执行

确认框给出三选一，光标默认停在第一项；标题会跟着高亮行变化，选之前就能看清后果：

| 选择 | 按键 | 效果 |
| --- | --- | --- |
| 会话和文件更改 | `1`、`y` | 把对话截断到该轮次，并把该轮次碰过的文件恢复到它运行之前的样子。该轮次新建的文件会被删除。 |
| 仅会话 | `2` | 只截断对话，磁盘上的文件一律不动。 |
| 仅文件更改 | `3` | 只恢复文件，一轮都不删，对话上下文保留。 |

`a` = 按高亮项执行并关闭**回退前确认**；`n` 或 **Esc** 取消；`j`/`k` 或方向键移动
光标，鼠标点击某一行即选中该行。选中「仅文件更改」时，标题会带上该轮次记录的文件
快照数量。

要选到「仅会话」或「仅文件更改」，**回退前确认**必须是开着的：关掉后，选择轮次会
立即按**会话和文件更改**执行，也就是确认框预选的那一项。

**重要：** 回退文件会用「该轮次运行前」的快照覆盖磁盘内容，你在 Grok 之外对这些
文件做的改动也会被覆盖。在意的话先提交或 stash。

### `/undo`

`/undo` 列出同一份回退点，把对话截断到你选的那一轮，但只回退对话：不弹确认、不选
模式，磁盘上的文件原样保留。需要连文件一起回退时用 `/rewind`。
...
- 使用 `/rewind` 撤销错误；它会把对话（以及你选择的那部分文件）回退到较早轮次。
