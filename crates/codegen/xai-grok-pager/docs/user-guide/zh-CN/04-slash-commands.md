...
将当前会话分支到新的智能体。未指定分叉点时，选择器会列出较早的用户提示，默认项保留整段对话。`--at <prompt>` 会立即在指定的第一条用户提示之前分叉，并把该提示预填到子会话输入框。`directive` 会作为子会话的首条提示发送，并优先于预填内容。位置 `1` 会得到空会话，因此会被拒绝；请改用 `/new`。

### `/rewind`

把会话回退到较早的轮次。默认开着**回退前确认**时，确认框里可以选择回退会话、回退该轮次碰过的文件，或两者一起回退。

### `/undo`

把对话回退到较早的轮次，磁盘上的文件一律不动。不会弹确认框。
